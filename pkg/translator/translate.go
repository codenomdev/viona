...
var Provider wire.ProviderSet = wire.NewSet(NewTranslator)
...
func NewTranslator(c *config.Config) (tr i18n.Translator, err error) {
	entries, err := os.ReadDir(c.I18N.BundleDir)
	if err != nil {
		return nil, err
	}
	...
	for _, file := range entries {
		...
		buf, err := os.ReadFile(filepath.Join(c.I18N.BundleDir, file.Name()))
		if err != nil {
	...
	i18nFile, err := os.ReadFile(filepath.Join(c.I18N.BundleDir, "i18n.yaml"))
	if err != nil {
