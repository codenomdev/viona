...
var (
	...
	GlobalValidatorMapping = make(map[i18n.Language]*MyValidator, 0)
)
...
// Check /
func (m *MyValidator) Check(value any) (err error) {
	errFields := []*FormErrorField{}
	defer func() {
		if len(errFields) == 0 {
			return
		}
	...
	if err != nil {
		...
		if !errors.As(err, &valErrors) {
			log.Error(err)
			return response.NewHttpConflict([]string{"validate check exception"})
		}
		...
		if len(errFields) > 0 {
			...
			// }
			return response.NewHttpBadRequest([]string{errMsg}, errFields)
		}
	}
